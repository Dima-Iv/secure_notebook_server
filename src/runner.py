        ...
        try:
            while True:
                ...
                if encrypted_message[0:1] == b'k':
                    ...
                    self.client_info.connection.send(self.client_info.encrypt_me('key expired'.encode()))
                elif encrypted_message[0:1] == b'l':
                    self.client_info.log_in(self.client_info.decrypt_me(encrypted_message[1:]))
                elif encrypted_message[0:1] == b'r':
                    self.client_info.registration(self.client_info.decrypt_me(encrypted_message[1:]))
                elif self.client_info.login == '':
                    self.client_info.connection.send(self.client_info.encrypt_me('Error: You are not login'.encode()))
                elif encrypted_message[0:1] == b'g':
                    self.client_info.get_notebook_list()
                elif encrypted_message[0:1] == b'o':
                    self.client_info.open_file(self.client_info.decrypt_me(encrypted_message[1:]))
                elif encrypted_message[0:1] == b's':
                    self.client_info.save_file(self.client_info.decrypt_me(encrypted_message[1:]))
                elif encrypted_message[0:1] == b'd':
                    self.client_info.delete_file(self.client_info.decrypt_me(encrypted_message[1:]))
